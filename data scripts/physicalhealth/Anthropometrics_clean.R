...
anthropometrics = load_instrument("abcd_ant01",physicalhealth_files_path)

anthropometrics = anthropometrics[,grepl("src|inter|sex|event|calc|cm", colnames(anthropometrics))] 

##### BMI ##### 

#get cdc percentiles table  
cdc_percentiles <- read_csv(paste0(main_abcd_path, abcd_box_path, "physicalhealth/bmiagerev.csv"))

#acording to cdc, add 0.5 to the age https://www.cdc.gov/nccdphp/dnpao/growthcharts/resources/sas.htm
anthropometrics$bmi_age_months = as.numeric(as.character(anthropometrics$interview_age)) + 0.5
...
#calculate BMI
BMI_dataset$anthroweightcalc = as.numeric(as.character(BMI_dataset$anthroweightcalc))
BMI_dataset$anthroheightcalc = as.numeric(as.character(BMI_dataset$anthroheightcalc))
